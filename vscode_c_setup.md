# VS Code C Compilation Setup and Troubleshooting
...
3. Once the installation finishes, check the box to "Run MSYS2 now". A terminal will open.
...
## Step 2: Add Compiler to Windows System Environment PATH
...
Here is the correct `.vscode/tasks.json` configuration for GCC (MinGW).

Create a folder named `.vscode` in your project folder, and inside it create a file named `tasks.json` with the following content:
```json
{
    "tasks": [
        {
